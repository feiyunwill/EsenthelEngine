...
void WorldView.updateWaypoint()
{
   ...
   if(mode()==WAYPOINT)
   {
      if(Edit.Viewport4.View *view=v4.getView(Gui.ms()))
      {
         ...
         if(lit_waypoint && dist>0.1)lit_waypoint=null;
         ...
         switch(waypoint_op())
         {
            ...
            case WPO_ROT       :
            {
                     ...
                     if(!p.removed)
               {
                  ...
                  if(!v4.perspective() || !v4.fpp())waypoint_edit_speed=CamMoveScale(v4.perspective());else if(first_push)waypoint_edit_speed=Dist(pos, ActiveCam.matrix.pos)*0.75; // in FPP mode automatically detect the speed according to waypoint distance to camera, but only at the moment of first button push
