...
class ObjPaintClass : PropWin
{
   ...
   static void Hide      (ObjPaintClass &op) {WorldEdit.obj_paint.set(false);}
   static Str  ElmName   (C UID &elm_id) {return Proj.elmFullName(elm_id);}
   static void ClearObjs (ObjPaintClass &op) {op.clearObjs();}
   static void RemoveObj (ptr   p   ) {ObjPaint.removeObj (intptr(p));}
   static void RemoveMtrl(Mtrl &mtrl) {ObjPaint.removeMtrl(mtrl);}
   static void GetMtrlA  (ptr) {ObjPaint.disallowed_mtrls_g.set(false, QUIET);}
   static void GetMtrlD  (ptr) {ObjPaint.   allowed_mtrls_g.set(false, QUIET);}
   static void Copy      (ObjPaintClass &op) {op. copyDo();}
   static void Paste     (ObjPaintClass &op) {op.pasteDo();}

   flt          collision=1, random_scale=0, scale_mul=1, align_normal=1, flow=0;
   ...
   bool ready    ()C {return !allowed_mtrls_g() && !disallowed_mtrls_g();}
   bool available()C {return WorldEdit.obj_paint() && !Selection.elms() && WorldEdit.mode()==WorldView.OBJECT;}
   void clearObjs()
   {
   ...
   void toGuiMtrls(Memx<Mtrl> &mtrls, Region &region)
   {
      ...
      FREPA(mtrls)
      {
         ...
         region+=mtrls[i].remove.create(Rect_RU(mtrls[i].rect().ru(), 0.04, 0.04)).func(RemoveMtrl, mtrls[i]).hide(); mtrls[i].remove.image="Gui/close.img";
      }
   }
   void  copyDo() {TextData t; FileText f; save(t); t.save(f.writeMem()); ClipSet(f.rewind().getAll());}
   void pasteDo() {TextData t; FileText f; f.writeMem().putText(ClipGet()); t.load(f.rewind()); load(t);}
   ...
   ObjPaintClass& create()
   {
      ListColumn lc_obj[]=
      {
         ListColumn(DATA_NONE, 0, 0, 0.045   , "remove"),
         ListColumn(ElmName        , LCW_DATA, "name"  ),
      };
      ts.reset().size=0.042; ts.align.set(1, 1);
      ...
      add("Scale Multiplier"    , MEMBER(ObjPaintClass, scale_mul   )).range(0.1, 10).desc("Constant scale factor applied to objects when painting").mouseEditMode(PROP_MOUSE_EDIT_SCALAR);
      add("Terrain Normal Align", MEMBER(ObjPaintClass, align_normal)).range(0  ,  1).desc("How much align the object to terrain normal vector\n0 = don't align\n1 = fully align").mouseEditSpeed(0.3);
      autoData(this);
      Rect r=super.create("Object Paint", Vec2(0.02, -0.01), 0.036, 0.043, 0.15); hide(); button[2].show().func(Hide, T);
      flt  h=ts.lineHeight(), w=0.46, p=h*1.3, y=r.min.y-p;
      T+=        objects_t    .create(Vec2(0.02, y), "Objects", &ts);
      T+=        objects_r    .create(Rect_LU(0.02, y, w, 0.25)); y=objects_r.rect().min.y-p;
      T+=        objects_clear.create(Rect_RD(objects_r.rect().ru(), 0.12, 0.045), "Clear").func(ClearObjs, T).desc("Remove all objects from the list");
      objects_r+=objects_list .create(lc_obj, Elms(lc_obj), true).elmHeight(0.036).textSize(0, 1).desc("Drag and drop objects here so they can be used during painting");
      T+=   allowed_mtrls_t.create(Vec2(0.02, y), "Allowed Materials", &ts);
      T+=   allowed_mtrls_r.create(Rect_LU(0.02, y, w, 0.16)).desc("Drag and drop materials here to specify on which terrain materials painting is allowed\nSet empty to allow painting on all materials"); y=allowed_mtrls_r.rect().min.y-p;
      ...
      T+=disallowed_mtrls_t.create(Vec2(0.02, y), "Disallowed Materials", &ts);
      T+=disallowed_mtrls_r.create(Rect_LU(0.02, y, w, 0.16)).desc("Drag and drop materials here to specify on which terrain materials painting is disallowed"); y=disallowed_mtrls_r.rect().min.y-p+h;
      ...
      T+=copy .create(Rect_U(0.02+w*0.25, y, 0.18, 0.05), "Copy" ).func(Copy , T);
      T+=paste.create(Rect_U(0.02+w*0.75, y, 0.18, 0.05), "Paste").func(Paste, T); y=copy.rect().min.y;
      Vec2 padd=defaultInnerPaddingSize()+0.02; rect(Rect_R(D.w(), 0, w+padd.x+0.02, -y+padd.y));
      return T;
   }
   virtual void update(C GuiPC &gpc)override
   {
      ...
      if(visible() && available())
      {
         if(ready())
         {
               ...
               if(Ms.b(1) || Ms.b(0))
            {
               ...
               flt  collision_radius=T.collision*0.5;
               ...
               flow+=Time.d()*Brush.speed*Brush.size*(area ? Mid(40/Sqrt(area), 0.1, 10.0) : 1); // make object insert speed dependent on object size
               ...
               for(; flow>=1; flow--)
               {
                  if(Ms.b(1)) // insert
                  {
                     ...
                     if(C ObjectPtr &base=object.obj)if(object.elm)
                     {
                        ...
                        {
                           flt src_radius=(base->mesh()->ext.size()*base->scale3()).xz().sum()*0.5*scale;
                           ...
                           REP(5) // 5 attempts to insert an object
                           {
                              ...
                              if(WorldEdit.cur.onHeightmap() || heightmap)
                              {
                                 ...
                                 if(Brush.slope_b())
                                 {
                                    flt s=1-WorldEdit.hmNormalAvg(Matrix(scale*0.5, pos), base->mesh() ? base->mesh()->ext : Extent(0.5)).y, slope=Brush.slope.cos(); // scale/2 is important especially for trees with big boxes
                                    if(Brush.slope.side ? s>slope : s<slope)goto collision_detected;
                                 }
                              }
                              if(collision_radius>EPS)
                              {
                                 Rect  rect =pos.xz(); rect.extend(src_radius*collision_radius + Sqrt(area)*0.5); // Sqrt(area)*0.5f = average object radius
                                 ...
                                 {
                                    flt test_radius=(obj.mesh->ext*obj.drawMatrix()).size().xz().sum()*0.5;
   ...
   bool     hasMtrl (Memx<Mtrl> &mtrls, C MaterialPtr &mtrl   )C {REPA(mtrls)if(mtrls[i].mtrl==mtrl   )return true; return false;}
   bool     hasMtrl (Memx<Mtrl> &mtrls, C UID         &mtrl_id)C {REPA(mtrls)if(mtrls[i].id  ==mtrl_id)return true; return false;}
   ...
   bool     hasObj (C UID &obj_id)C {REPA(objects_data)if(objects_data[i].id==obj_id)return true; return false;}
