...
//#include <algorithm> ESENTHEL
//#include <limits> ESENTHEL
// Code known to compile and run with Qt 4.3 through Qt 4.7.
//#include <QtCore> ESENTHEL
//#include <time.h> ESENTHEL
...
/*QString Diff::strOperation(Operation op) {
  ...
  // throw "Invalid operation."; ESENTHEL
  DEBUG_ASSERT(false, "Invalid operation"); return QString();
}*/
...
QString Patch::toString()C {
  ...
  //foreach (Diff aDiff, diffs) { ESENTHEL
  FREPA(diffs) {C Diff &aDiff=diffs[i];
...
QList<Diff> diff_match_patch::diff_main(const QString &text1,
  ...
  //if (text1.isNull() || text2.isNull()) {throw "Null inputs. (diff_main)";} ESENTHEL
      ...
      case EQUAL:
        ...
        if (count_delete >= 1 && count_insert >= 1) {
          ...
          /*foreach(Diff newDiff,
              diff_main(text_delete, text_insert, false, deadline)) { ESENTHEL */
          C QList<Diff> &newDiffs=diff_main(text_delete, text_insert, false, deadline);
          FREPAD(j, newDiffs)
          {
            C Diff &newDiff=newDiffs[j];
...
int diff_match_patch::diff_commonPrefix(const QString &text1,
  ...
  const int n = Min(text1.length(), text2.length());
  for (int i = 0; i < n; i++) {
    if (text1[i] != text2[i]) {
      return i;
    }
...
int diff_match_patch::diff_commonSuffix(const QString &text1,
  ...
  const int text1_length = text1.length();
  const int text2_length = text2.length();
  const int n = Min(text1_length, text2_length); // ESENTHEL
  for (int i = 1; i <= n; i++) {
...
int diff_match_patch::diff_commonOverlap(const QString &text1,
  ...
  const int text_length = Min(text1_length, text2_length);
  ...
  while (thisDiff != NULL) {
    if (thisDiff->operation == EQUAL) {
      ...
      if (!lastequality.isNull()
          ...
          && (lastequality.length()
              <= Max(length_insertions2, length_deletions2))) { // ESENTHEL
...
int diff_match_patch::diff_cleanupSemanticScore(const QString &one,
  ...
  bool lineBreak1 = whitespace1 && char1.isControl(); // category() == QChar::Other_Control; ESENTHEL
  bool lineBreak2 = whitespace2 && char2.isControl(); // category() == QChar::Other_Control; ESENTHEL
  bool blankLine1 = lineBreak1 && one.isBlankLineEnd  (); // BLANKLINEEND  .indexIn(one) != -1; ESENTHEL
  bool blankLine2 = lineBreak2 && two.isBlankLineStart(); // BLANKLINESTART.indexIn(two) != -1; ESENTHEL
          ...
          if (both_types) {
            ...
            if (commonlength != 0) {
              if (pointer.hasPrevious()) {
                thisDiff = &pointer.previous();
                //if (thisDiff->operation != EQUAL) {throw "Previous diff should have been an equality.";} ESENTHEL
                DEBUG_ASSERT(thisDiff->operation==EQUAL, "Previous diff should have been an equality.");
  ...
  FREPA(diffs) {C Diff &aDiff=diffs[i];
    switch (aDiff.operation) {
      ...
      case EQUAL:
        ...
        levenshtein += Max(insertions, deletions); // ESENTHEL
        insertions = 0;
        deletions = 0;
        break;
    }
...
QList<Diff> diff_match_patch::diff_fromDelta(const QString &text1,
  ...
  //foreach(QString token, tokens) { ESENTHEL
  FREPA(tokens) {C QString &token=tokens[i];
    if (token.isEmpty()) {
    ...
    switch (token[0].toAscii()) {
      case '+':
        param = QUrl::fromPercentEncoding(param); // qPrintable(param) ESENTHEL
      ...
      case '=': {
        int n;
        n = param.toInt();
        //if (n < 0) throw QString("Negative number in diff_fromDelta: %1").arg(param); ESENTHEL
        DEBUG_ASSERT(n>=0, "Negative number in diff_fromDelta");
        QString text;
      ...
      default:
        //throw QString("Invalid diff operation in diff_fromDelta: %1").arg(token[0]); ESENTHEL
        DEBUG_ASSERT(false, "Invalid diff operation in diff_fromDelta");
    }
  }
  //if (pointer != text1.length())throw QString("Delta length (%1) smaller than source text length (%2)").arg(pointer).arg(text1.length()); ESENTHEL
  DEBUG_ASSERT(pointer==text1.length(), "Delta length smaller than source text length");
  return diffs;
}*/
...
int diff_match_patch::match_main(const QString &text, const QString &pattern,
                                 int loc) {
  // Check for null inputs.
  //if (text.isNull() || pattern.isNull()) {throw "Null inputs. (match_main)";} ESENTHEL

  loc = Max(0, Min(loc, text.length())); // ESENTHEL
  if (text == pattern) {
...
int diff_match_patch::match_bitap(const QString &text, const QString &pattern,
                                  int loc) {
  //if (!(Match_MaxBits == 0 || pattern.length() <= Match_MaxBits))throw "Pattern too long for this application."; // ESENTHEL
  DEBUG_ASSERT(Match_MaxBits == 0 || pattern.length() <= Match_MaxBits, "Pattern too long for this application.");
  ...
  if (best_loc != -1) {
    score_threshold = Min(match_bitapScore(0, best_loc, loc, pattern),
        score_threshold);
...
void diff_match_patch::patch_addContext(Patch &patch, const QString &text) {
  ...
  while (text.indexOf(pattern) != text.lastIndexOf(pattern)
    ...
    pattern = safeMid(text, Max(0, patch.start2 - padding),
  ...
  // Add the prefix.
  QString prefix = safeMid(text, Max(0, patch.start2 - padding),
...
QList<Patch> diff_match_patch::patch_make(const QString &text1,
                                          const QString &text2) {
  // Check for null inputs.
  //if (text1.isNull() || text2.isNull()) {throw "Null inputs. (patch_make)";}
...
QList<Patch> diff_match_patch::patch_make(const QString &text1,
                                          const QList<Diff> &diffs) {
  // Check for null inputs.
  //if (text1.isNull()) {throw "Null inputs. (patch_make)";}
...
QList<Patch> diff_match_patch::patch_deepCopy(QList<Patch> &patches) {
  ...
  //foreach(Patch aPatch, patches) { ESENTHEL
  FREPA(patches) {C Patch &aPatch=patches[i];
    Patch patchCopy = Patch();
    //foreach(Diff aDiff, aPatch.diffs) { ESENTHEL
    FREPA(aPatch.diffs) {C Diff &aDiff=aPatch.diffs[i];
...
QPair<QString, QVector<bool> > diff_match_patch::patch_apply(
  ...
  //foreach(Patch aPatch, patchesCopy) { ESENTHEL
  FREPA(patchesCopy) {C Patch &aPatch=patchesCopy[i];
    ...
    while (!bigpatch.diffs.isEmpty()) {
      ...
      while (!bigpatch.diffs.isEmpty()
        ...
        if (diff_type == INSERT) {
          ...
          diff_text = diff_text.left(Min(diff_text.length(),
...
QList<Patch> diff_match_patch::patch_fromText(const QString &textline) {
  ...
  QString line;
  while (!text.isEmpty()) {
    //if (!patchHeader.exactMatch(text.front()))throw QString("Invalid patch string: %1").arg(text.front()); ESENTHEL
    DEBUG_ASSERT(patchHeader.exactMatch(text.front()), "Invalid patch string");
    ...
    while (!text.isEmpty()) {
      ...
      line = QUrl::fromPercentEncoding(line); // qPrintable(line) ESENTHEL
      if (sign == '-') {
        // Deletion.
        ...
        // WTF?
        // throw QString("Invalid patch mode '%1' in: %2").arg(sign).arg(line); ESENTHEL
        DEBUG_ASSERT(false, "Invalid patch mode");
        return QList<Patch>();
      }
      text.removeFirst();
    }
