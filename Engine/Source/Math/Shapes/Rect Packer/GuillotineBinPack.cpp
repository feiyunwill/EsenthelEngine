...
GuillotineBinPack::GuillotineBinPack(int width, int height, bool allowRotate)
{
	Init(width, height, allowRotate);
}

void GuillotineBinPack::Init(int width, int height, bool allowRotate_)
{
	binWidth = width;
	binHeight = height;
...
void GuillotineBinPack::Insert(MemPtr<RectSizeIndex> rects, MemPtr<RectIndex> dst, bool merge, 
		...
		for(Int i = 0; i < freeRectangles.elms(); ++i)
		{
			for(Int j = 0; j < rects.elms(); ++j)
			{
				...
				if (rects[j].width == freeRectangles[i].width && rects[j].height == freeRectangles[i].height)
				{
					...
					bestFlipped = false;
					bestScore = INT_MIN;
					i = freeRectangles.elms(); // Force a jump out of the outer loop as well - we got an instant fit.
					break;
				}
				// If flipping this rectangle is a perfect match, pick that then.
				else if (allowRotate && rects[j].height == freeRectangles[i].width && rects[j].width == freeRectangles[i].height)
				{
					bestFreeRect = i;
					bestRect = j;
					bestFlipped = true;
...
bool Fits(const RectSize &r, const Rect &freeRect, bool allowRotate)
{
	return (r.width <= freeRect.width && r.height <= freeRect.height) ||
		(allowRotate && r.height <= freeRect.width && r.width <= freeRect.height);
}

/// @return True if r fits perfectly inside freeRect, i.e. the leftover area is 0.
bool FitsPerfectly(const RectSize &r, const Rect &freeRect, bool allowRotate)
{
	return (r.width == freeRect.width && r.height == freeRect.height) ||
		(allowRotate && r.height == freeRect.width && r.width == freeRect.height);
}
	...
	FreeRectChoiceHeuristic rectChoice, GuillotineSplitHeuristic splitMethod)
{
	...
	while(rects.elms() > 0 && freeRectangles.elms() > 0)
	{
		int bestScore1 = -1;
		int bestScore2 = -1;

		///\todo Different sort predicates.
		clb::sort::QuickSort(&freeRectangles[0], freeRectangles.elms(), CompareRectShortSide);
...
Rect GuillotineBinPack::FindPositionForNewNode(int width, int height, FreeRectChoiceHeuristic rectChoice, int *nodeIndex)
{
	...
	for(Int i = 0; i < freeRectangles.elms(); ++i)
	{
		...
		else if (allowRotate && height == freeRectangles[i].width && width == freeRectangles[i].height)
		{
			bestNode.x = freeRectangles[i].x;
			bestNode.y = freeRectangles[i].y;
...
void GuillotineBinPack::MergeFreeList()
{
	...
	for(Int i = 0; i < freeRectangles.elms(); ++i)
		debug_assert(test.Add(freeRectangles[i]) == true);
#endif
